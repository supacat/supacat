...
- PRs land by squash merge only — merge commits and rebase merges are disabled on GitHub. The squash commit takes the PR title as its subject and the PR body as its message, so title and body are what `main` records. Open the PR ready for review, never as a draft; `joulse` merges, and the head branch is deleted on merge.
